...
Obtenemos que cada sección medirá `r parte` grados de latitud. 

Si empezamos desde el norte del país (`n`, que equivale a `r round(limite_norte, 1)`), la primera sección llegará hasta `n - parte` (`r round(limite_norte - parte, 1)`). Luego la segunda sección empieza donde termina la anterior, y llega hasta `n - parte*2` (`r round(limite_norte - parte * 2)`), y la tercera sección llegará hasta `n - parte*2` (`r limite_norte - parte * 3`), que coincide con el límite sur del país.
