...
Transformemos los datos censales para expresar la desocupación por medio de puntos, pero como los datos vienen en manzanas, hagamos que cada punto represente una suma de varias manzanas. Para eso, agruparemos los datos por comuna y localidad con `group_by()`, luego [haremos un resumen de datos](/blog/r_introduccion/dplyr_summarize/) con `summarize()` donde sumaremos las variables y uniremos las manzanas en un sólo polígono/fila con `st_union()`, y [calcularemos el centroide](/blog/mapas_sf/#calcular-centroide) de los polígonos con `st_centroid()` para obtener los puntos, de manera que un punto resuma los datos de todas las manzanas de la misma localidad.
...
Para que el **radio** de cada círculo dependa de una variable numérica, usamos la función `interpolate()` para hacer que los valores de la variable `n_desocupado` se expresen en distintos tamaños de los círculos:
